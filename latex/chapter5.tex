...
%% FIXME: Falta introduzir aqui porque é que vais falar de arquiteturas de oracles neste ponto. Já hás-de ter falado disto mais acima no documento, mas aqui é importante para o que se segue não parecer caído do céu. 

%% FIXME: Outra coisa que faz falta aqui é uma discussão geral do problema (necessidade de ter informação exogenea na blockchain) e de como é que os oracles o resolvem. Isto também já há-de ter sido descrito de alguma forma mais acima, mas aqui poderia aparecer num formato parecido ao que usas em baixo para descrever cada uma das arquiteturas (context, example, problem, forces, solution, example resolved, resulting context, known uses) e possivelmente com mais detalhe. Podes inclusive dizer que estás a descrever isto como um padrão de software chamado "Blockchain Oracle", porque a existência de oracles é algo que vemos recorrentemente e para a qual até temos vários "know uses". O mesmo pode não acontecer com as arquiteturas que descreves a seguir, e daí não ser adequado chamar-lhes padrões.
...
%% estas forças também me parecem um pouco genéricas para esta arquitetura. as forças são as restrições que consideramos quando procuramos pela solução para um problema. ajudam-nos o leitor a compreender a escolha da solução
