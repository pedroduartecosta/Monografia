...
% FIXME: Falta explicar a importância de authenticity proofs e  fazer a ligação com o que será dito no capítulo da introdução sobre isso
...
%FIXME: rever a frase. a quantidade de dados tb se torna mainstream?!
...
% FIXME: clarificar porquê que se torna crucial. qual a consequência se a autenticação da informação não existir?
...
% FIXME: Seria excelente dar aqui alguns exemplos de TEEs, à semelhança do que se faz na secção seguinte para as APs. Esta secção precisa de referências.
...
TLSNotary provides some capabilities to attest TLS connections but comes with several limitations. Firstly, TLSNotary supports only TLS 1.0 or 1.1, the properties mentioned before are not present in TLS 1.2 and 1.3 and the former are considered less secure versions of TLS. Secondly, TLSNotary depends on RSA Key exchange, which does not provide forward secrecy. Thirdly, TLSNotary uses MD5 and SHA-1 functions, which are now considered deprecated. Finally and most importantly, TLSNotary requires trusting in a third party in most of its implementations, such as in Oraclize \cite{Oraclize.it2018OraclizeDocumentation}, and being an interactive proof there is no way to verify the TLSNotary proof unless you were performing the role of the auditor during the retrieval. Oraclize, runs an auditor node on Amazon Web Services (AWS), claiming that this implementation is secure as far as AWS is trusted, simply moving the trust to a larger central entity. It also only allows the existence of one auditor in which we must trust, TLSNotary will not be a suitable solution if more than one auditor is required.
...
%FIXME: Explain what non-repudiation is
...
%FIXME: Add diagram or explain the previous sentence
