...
\begin{tikzpicture}[>=latex]
    ...
    \coordinate (D) at (6,0);
    \draw[thick] (A)--(B) (C)--(D);
    \draw (A) node[above]{\Large Requester};
    \draw (C) node[above]{\Large Generator};

    \coordinate (Q) at ($(C)!.1!(D)$);
    \coordinate (R) at ($(A)!.1!(B)$);

    \draw[->] (Q) -- (R) node[midway,sloped,above]{\verb$TLS Handshake$};
    \draw[->] (R) -- (Q) node[midway,sloped,above]{\verb$TLS Handshake$};

    \coordinate (E) at ($(A)!.25!(B)$);
    \coordinate (F) at ($(C)!.35!(D)$);
    \coordinate (G) at ($(A)!.35!(B)$);
    \coordinate (H) at ($(C)!.45!(D)$);

    \draw[->] (E) -- (F) node[midway,sloped,above]{\verb$Req x$};
    \draw[loosely dashed, ->] (G) -- (H) node[midway,sloped,above]{\verb$Collect Evidence$};

    \coordinate (I) at ($(A)!.45!(B)$);
    \coordinate (J) at ($(C)!.55!(D)$);
    \coordinate (K) at ($(A)!.55!(B)$);
    \coordinate (L) at ($(C)!.65!(D)$);

    \draw[->] (I) -- (J) node[midway,sloped,above]{\verb$Req y$};
    \draw[loosely dashed, ->] (K) -- (L) node[midway,sloped,above]{\verb$Return Evidence$};
    ...
    \draw[->] (M) -- (N) node[midway,sloped,above]{\verb$123$};
    \draw[->] (O) -- (P) node[midway,sloped,above]{\verb$987$};
