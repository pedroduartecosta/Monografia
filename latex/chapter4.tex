...
\subsection{Trusting data}
By querying several data sources there is a higher trust on the veracity of the data.

\subsection{Higher trust on oracle behaviour}
Oracle good behaviour is usually backed by authenticity proofs which are expensive to check on-chain or don't bring much value to the contract when verified off-chain since the current contract already executed its code with tampered data.

\subsection{Lower smart contract costs}
Checking authenticity proofs leads to higher contract deployment costs, as the proof can be long and computationally expensive.

\subsection{Lower smart contract complexity}
Verifying authenticity proofs on chain requires the developer to have sufficient knowledge to write the verifying functions.
...
Having one party control the oracle network central-izes the power to manipulate all the contracts relying on the information provided by thatnetwork of oracles.
